...
int main (int argc, char * argv[]){
    ...
    for (int i = 0; i< 3; i++){
        sprintf (s, "%d_YPMSD%d.in", K, i + 1);
        fin = fopen (s, "r");
        printf ("%s", s);
        int pd, pk, pz;
        fscanf (fin, "%d%d%d", &pd, &pk, &pz);
        printf ("%d %d %d ", pd, pk, pz);
        double ** tmpc = (double **)malloc(sizeof(double *) * (pk + pz));
        for (int j = 0; j < pk + pz; j ++){
            tmpc[j] = (double *)malloc(sizeof(double) * pd);
            for (int k = 0; k < pd; k ++){
                fscanf (fin, "%lf", &(tmpc[j][k]));
            }
        }
    	for (int j = 0; j < PTS; j ++) {
            int gr;
